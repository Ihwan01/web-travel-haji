        ...
        <form action="<?= base_url('company/update') ?>" method="POST">
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <textarea name="whatsapp_message" class="form-control" rows="2" placeholder="Assalamu'alaikum Nuansa Rindu, saya ingin bertanya tentang..."><?= htmlspecialchars($company->whatsapp_message ?? '') ?></textarea>
                ...
                <div class="col-md-12 mb-4">
                    ...
                    <textarea name="google_maps_iframe" class="form-control text-monospace text-sm" rows="4" placeholder='<iframe src="..."></iframe>'><?= htmlspecialchars($company->Maps_iframe ?? '') ?></textarea>
                    <small class="text-muted">Buka Google Maps > Bagikan > Sematkan Peta > Salin HTML.</small>
                </div>
            </div>

            <button type="submit" class="btn btn-primary px-4 py-2" id="btnSubmitCompany">
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // Pantau setiap ketikan / perubahan pada input, textarea, select
        ...
        function setDirty() {
            if (!formChanged) {
                formChanged = true;
                // Ubah tombol jadi warna peringatan
                submitBtn.classList.remove('btn-primary');
                submitBtn.classList.add('btn-warning', 'text-dark', 'fw-bold', 'shadow-sm');
                submitBtn.innerHTML = '<i class="fas fa-exclamation-triangle text-danger mr-2"></i> Ada Perubahan (Belum Disimpan)';
            }
        }

        // Jika form disubmit secara sengaja, hapus peringatan
        form.addEventListener('submit', function() {
            formChanged = false;
            submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i> Menyimpan...';
        });

        // Munculkan peringatan bawaan browser jika tab ditutup mendadak
        window.addEventListener('beforeunload', function(e) {
            if (formChanged) {
                e.preventDefault();
                e.returnValue = 'Anda memiliki perubahan kontak yang belum disimpan!';
            }
        });
    });
</script>